...
erfexp = TF1('erfexp', '[0]*TMath::Erf((x-[1])/[2])*exp(-x/[3])',low,high)
erfexp2 = TF1('erfexp2', '[0]*TMath::Erf((x-[1])/[2])*exp(-x/[3])',low,high)

#fit both histograms
#Guess
erfexp.SetParameters(250,3,4,5)
...
#guess
erfexp2.SetParameters(160,3.5,5,2)
#fix the X intercept of erfexp
#erfexp2.SetParLimits(1,3.31,3.41)
erfexp2.FixParameter(1,3.28)
#We want to push the right sideband fit up
#erfexp2.SetParLimits(3,1.,10)
#erfexp2.SetParLimits(0,2000,10000)
#fit
hright.Fit('erfexp2','M','',low,high)
...
rightfit = hright.GetFunction('erfexp2')
rightfit.SetLineColor(3)

leftN = hleft.Integral(hleft.FindBin(3.3),hleft.FindBin(7.8))
rightN = hright.Integral(hright.FindBin(3.3),hright.FindBin(7.8))
...
arithSignal_fit.SetParameters(arithavgp0,arithavgp1,arithavgp2,arithavgp3)
...
geoSignal_fit.SetParameters(geoavgp0,geoavgp1,geoavgp2,geoavgp3)
...
#integrate
intleft = leftfit.Integral(3.3,7.8)
intright = rightfit.Integral(3.3,7.8)
intgeosig = geoSignal_fit.Integral(3.3,7.8)
intarithsig = arithSignal_fit.Integral(3.3,7.8)
...
print('Integral of the normalized left fit function',normleftfit.Integral(3.3,7.8))
...
for bias in biases:
    ...
    biasSignal_fit.SetParameters(biasavgp0,biasavgp1,biasavgp2,biasavgp3)
    intbias = biasSignal_fit.Integral(3.3,7.8)
...
sum_fit = TF1('sum_fit','[0]*TMath::Erf((x-[1])/[2])*exp(-x/[3]) + [4]*TMath::Erf((x-[5])/[6])*exp(-x/[7])',low,high)
...
sum_fit.SetParameter(4,rightfit.GetParameter(0))
sum_fit.SetParameter(5,rightfit.GetParameter(1))
sum_fit.SetParameter(6,rightfit.GetParameter(2))
sum_fit.SetParameter(7,rightfit.GetParameter(3))
sum_fit.GetHistogram().SetTitle('')

#Sum the histograms and draw this as well.
hsum = hleft+hright
datanormsigfit = copy.deepcopy(normarithsigfit)
Nleft = hleft.Integral(hleft.FindBin(3.3),hleft.FindBin(7.8))
Nright = hright.Integral(hright.FindBin(3.3),hright.FindBin(7.8))
Nsum = hsum.Integral(hsum.FindBin(3.3),hsum.FindBin(7.8))

datanormsigfit.SetParameter(0,datanormsigfit.GetParameter(0) * Nsum*hleft.GetBinWidth(3) / datanormsigfit.Integral(3.3,7.8))
sum_fit.SetParameter(0,sum_fit.GetParameter(0) * Nsum*hleft.GetBinWidth(3) / sum_fit.Integral(3.3,7.8))
...
"""#normalize the sum
intsum = sum_fit.Integral(3.3,7.8)
