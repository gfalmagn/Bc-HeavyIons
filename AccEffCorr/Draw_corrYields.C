...
void Draw_corrYields(bool ispp=true, bool secondStep=true, bool thirdStep=false){
  ...
  if(secondStep){
    ...
    infile2->GetObject("efficiency_oneBinned"+(TString)(ispp?"_pp":"_PbPb")+(TString)"_2ndStep", eff_oneBinned_2ndStep);
  }

  TFile *infile4 = (TFile*)(thirdStep?(new TFile("../twoSteps/AccEffFrom2ndStepToys.root","READ")):NULL);
  if(thirdStep) infile4->GetObject("InvAccEffFromCorrMC_withSystErr_"+(TString)(ispp?"pp":"PbPb")+(TString)(secondStep?"_2ndStep":""), InvAccEff);
  ...
  for(int b=1;b<=nbins;b++){
    ...
    cout<<"one-binned acceptance, efficiency = "<<(*acc_oneBinned)[1-(int)ispp][b]<<" "<<(*eff_oneBinned)[b][0]<<endl;
    ...
    x_3steps[b-1] = x[b-1] + 0.24*((b==1)?3:1)*xErr[b-1];
    zero[b-1] = 0;
    ...
    y_oneBinned[b-1] = (*Yields_postfit)[0][b][0] / ((*acc_oneBinned)[1-(int)ispp][b] * (*eff_oneBinned)[b][0]);
    yErr[b-1] = (*rsig_relerr)[b][0] * y_oneBinned[b-1];
    if(secondStep){
      y_2steps[b-1] = (*Yields_postfit)[0][b][0]  / ((*acc_oneBinned_2ndStep)[1-(int)ispp][b] * (*eff_oneBinned_2ndStep)[b][0]);
      yErr_2steps[b-1] = (*rsig_relerr)[b][0] * y_2steps[b-1];
    }
    if(thirdStep){
      y_3steps[b-1] = (*Yields_postfit)[0][b][0] * (*InvAccEff)[b-1][0];
      yErr_3steps[b-1] = (*Yields_postfit)[0][b][0] * (*InvAccEff)[b-1][1];//sqrt(pow((*Yields_postfit)[0][b][0] * (*InvAccEff)[b-1][1], 2) + pow( (*rsig_relerr)[b][0] * y_2steps[b-1] ,2));
    }
    ...
    cout<<"Yields_postfit, InvAccEff (if 3rd step), y_2steps, yErr_2steps = "<<(*Yields_postfit)[0][b][0]<<" "<<(thirdStep?((*InvAccEff)[b-1][0]):0)<<" "<<y_2steps[b-1]<<" "<<yErr_2steps[b-1]<<endl;
  ...
  TGraphErrors *g_3steps = new TGraphErrors(nbins, x_3steps,y_3steps,zero,yErr_3steps);
