...
void BuildEffMap(bool ispp = true, bool BDTuncorrFromM=false, bool integratePtBins=false){
  ...
  TH1F *hist_corr = new TH1F("hist_corr", "distribution des corrections 'compliquees'", 100, 0, 4.4);
  TH1F *hist_ratio = new TH1F("hist_ratio", "distribution des ratios des 2 types de corrections", 100, 0, 4.4); 
  TH1F *hist_SF = new TH1F("hist_SF", "distribution des SF", 100, 0, 4.4);
  TH1F *hist_LH = new TH1F("hist_LH", "distribution des L/H", 100, 0, 4.4);
  */
  //**************************************************************  
  //Get input branches
  ...
  /* Some verifications
    TH2Poly *hp_corr = (TH2Poly*) hp->Clone("hp_corr");
    TH2Poly *hp_weight = (TH2Poly*) hp->Clone("hp_weight");
    TH2Poly *hp_ratio = (TH2Poly*) hp->Clone("hp_ratio");
  */
  ...
  for(int j=0; j<nentries; j++){//nentries
	...
	if(QQidx>-1){
	  ...
	  bool muW_L2 = (Reco_mu_trig[muWidx]&(65536))>0 ;
	  bool muW_L3 = (Reco_mu_trig[muWidx]&(131072))>0 ;
	  ...
	  bool mumi_L2 = (Reco_mu_trig[mumiidx]&(65536))>0 ;
	  bool mumi_L3 = (Reco_mu_trig[mumiidx]&(131072))>0 ;
	  ...
	  bool mupl_L2 = (Reco_mu_trig[muplidx]&(65536))>0 ;
	  bool mupl_L3 = (Reco_mu_trig[muplidx]&(131072))>0 ;
	  ...
	  if(
	    ...
	    //H = tight criteria (tight acceptance, triggering muon...) L = loose criteria (loose acceptance, no trigger required...)
	    //2 muons have to respect H, 1 has to respect L
	    //muon 1 is mumi, muon 2 is mupl, muon3 is muW    
	    double SF1_H,SF2_H,SF3_H,SF1_L,SF2_L,SF3_L; //Definition of the scale-factor for the 3 muons, L or H criteria
	    double L1,L2,L3,H1,H2,H3;//Definition of the MC efficiencies
	    double effcorr,simple; //effcorr is the correction to be applied, simple is a simpler correction = mean of the 3 SF for the 3 muons
	    int filt1,filt2,filt3; //if 0 : L2 filter, if 1 : L3 filter
	    double trg1, trg2, trg3;//Will be used for the SF of triggering muons in PbPb 
	    double termA, termB, termC;//Will be used in PbPb for easier writting of the correction formula
	    if (!ispp){//Case PbPb
	      SF1_H = tnp_weight_muid_pbpb(mumi_pt,mumi_eta,0)*tnp_weight_trk_pbpb(mumi_eta,0);//Product of muid and trk, tight acceptance
	      SF2_H = tnp_weight_muid_pbpb(mupl_pt,mupl_eta,0)*tnp_weight_trk_pbpb(mupl_eta,0);
	      SF3_H = tnp_weight_muid_pbpb(muW_pt,muW_eta,0)*tnp_weight_trk_pbpb(muW_eta,0);
	      H1 = tnp_weight_muid_pbpb(mumi_pt,mumi_eta,3)*tnp_weight_trk_pbpb(mumi_eta,3);
	      H2 = tnp_weight_muid_pbpb(mupl_pt,mupl_eta,3)*tnp_weight_trk_pbpb(mupl_eta,3);
	      H3 = tnp_weight_muid_pbpb(muW_pt,muW_eta,3)*tnp_weight_trk_pbpb(muW_eta,3);
	      SF1_L = tnp_weight_muid_looseacceptance_pbpb(mumi_pt,mumi_eta,0)*tnp_weight_trk_looseacceptance_pbpb(mumi_eta,0);//Product of muid and trk, loose acceptance
              ...
              L1 = tnp_weight_muid_looseacceptance_pbpb(mumi_pt,mumi_eta,3)*tnp_weight_trk_looseacceptance_pbpb(mumi_eta,3);
	      ...
	      //Case where we apply H to mupl and muW
	      if ((mupl_L3)&&(!muW_L3)){//one respects the L3 filter and not the other, we apply the SF for L3 to the one that passes it and the SF for L2 to the other one
		filt2 = 1; 
		filt3 = 0;}
	      else if ((!mupl_L3)&&(muW_L3)){
		filt2 = 0;
		filt3 = 1;}
	      else if ((mupl_L3)&&(muW_L3)){//if both muons pass the L3 filter, choose 1 randomly to apply the SF for L3, and apply the SF for L2 to the other one
		TRandom* rn = new TRandom();
		filt2 = rn->Integer(1);
		if (filt2 == 0) {filt3 = 1;}
		else if (filt2 == 1) {filt3 = 0;}
	      }
	      else {cout << "None of the muons passes the L3 filter" << endl;}//This case is normaly impossible
	      trg2 = tnp_weight_trg_pbpb(mupl_pt,mupl_eta,filt2,0);
	      trg3 = tnp_weight_trg_pbpb(muW_pt,muW_eta,filt3,0);
	      termA = trg2*trg3;

	      //Case where H is applied to mumi and muW
              if ((mumi_L3)&&(!muW_L3)){
                filt1 = 1;
                filt3 = 0;}
              else if ((!mumi_L3)&&(muW_L3)){
                filt1 = 0;
                filt3 = 1;}
              else if ((mumi_L3)&&(muW_L3)){
                TRandom* rn = new TRandom();
                filt1 = rn->Integer(1);
                if (filt1 == 0) {filt3 = 1;}
                else if (filt1 == 1) {filt3 = 0;}
              }
              else {cout << "None of the muons passes the L3 filter" << endl;}
              trg1 = tnp_weight_trg_pbpb(mumi_pt,mumi_eta,filt1,0);
              trg3 = tnp_weight_trg_pbpb(muW_pt,muW_eta,filt3,0);
              termB =trg1*trg3;

	      //Case where H is applied to mumi and mupl
              if ((mumi_L3)&&(!mupl_L3)){
                filt1 = 1;
                filt2 = 0;}
              else if ((!mumi_L3)&&(mupl_L3)){
                filt1 = 0;
                filt2 = 1;}
              else if ((mumi_L3)&&(mupl_L3)){
                TRandom* rn = new TRandom();
	      ...
	      //If the three muons respect H, we apply the more complicated correction formula
	      if (mumi_inTightAcc && mumi_trig && mupl_inTightAcc && mupl_trig && muW_inTightAcc && muW_trig){
		effcorr = (((L1/H1)*(SF1_L*SF2_H*SF3_H*termA))+((L2/H2)*(SF1_H*SF2_L*SF3_H*termB))+((L3/H3)*(SF1_H*SF2_H*SF3_L*termC))-2)/((L1/H1)+(L2/H2)+(L3/H3)-2);
		simple = (((SF1_L*SF2_H*SF3_H*termA)+(SF1_H*SF2_L*SF3_H*termB)+(SF1_H*SF2_H*SF3_L*termC))/3);//Simple mean of the SF for comparison
		//hist_corr->Fill(effcorr,1); Checking the corrections
		//hist_ratio->Fill((simple/effcorr),1); Comparing the two types of correction

	      }
	      //If one of the muons does not respect H, then the formula is reduced to a simple product of SF, and the two type of corrections are equal
	      else if (mumi_inTightAcc && mumi_trig && mupl_inTightAcc && mupl_trig && (!muW_inTightAcc || !muW_trig)){effcorr = SF1_H*SF2_H*SF3_L*termC; simple = effcorr;}
	      else if (mumi_inTightAcc && mumi_trig && muW_inTightAcc && muW_trig && (!mupl_inTightAcc || !mupl_trig)){effcorr = SF1_H*SF2_L*SF3_H*termB; simple = effcorr;}
	      else if (mupl_inTightAcc && mupl_trig && muW_inTightAcc && muW_trig && (!mumi_inTightAcc || !mumi_trig)){effcorr = SF1_L*SF2_H*SF3_H*termA; simple = effcorr;}  
	    }//end PbPb
	    else {//Case pp
	      termA = 1;//we do not need the L2 and L3 filters in pp
	      termB = 1;
	      termC = 1;            
	      ...
	      */
	      ...
	      //If the three muons respect H
              if (mumi_inTightAcc && mumi_trig && mupl_inTightAcc && mupl_trig && muW_inTightAcc && muW_trig){
                effcorr = (((L1/H1)*SF1_L*SF2_H*SF3_H)+((L2/H2)*SF1_H*SF2_L*SF3_H)+((L3/H3)*SF1_H*SF2_H*SF3_L)-2)/((L1/H1)+(L2/H2)+(L3/H3)-2);
		simple = (((SF1_L*SF2_H*SF3_H)+(SF1_H*SF2_L*SF3_H)+(SF1_H*SF2_H*SF3_L))/3);
		//hist_corr->Fill(effcorr);
		//hist_ratio->Fill((simple/effcorr));

              }
	      //If only 2 muons respect H
              else if (mumi_inTightAcc && mumi_trig && mupl_inTightAcc && mupl_trig && (!muW_inTightAcc || !muW_trig)){effcorr = SF1_H*SF2_H*SF3_L; simple = effcorr;}
              else if (mumi_inTightAcc && mumi_trig && muW_inTightAcc && muW_trig && (!mupl_inTightAcc || !mupl_trig)){effcorr = SF1_H*SF2_L*SF3_H; simple = effcorr;}
              else if (mupl_inTightAcc && mupl_trig && muW_inTightAcc && muW_trig && (!mumi_inTightAcc || !mumi_trig)){effcorr = SF1_L*SF2_H*SF3_H; simple = effcorr;}
	    }//end pp
	     
	    for(int b=1;b<=_NanaBins;b++){
	      if(fabs(gen3mu->Rapidity())>_BcYmin[b] && fabs(gen3mu->Rapidity())<_BcYmax[b] && gen3mu->Pt()>_BcPtmin[b] && gen3mu->Pt()<_BcPtmax[b] ){
		passing_oneBinned[b] += effcorr*weight;
		passing_oneBinned[0] += effcorr*weight; //Corrections are applied here
	      }
	    }
	    hp_sel->Fill(fabs(gen3mu->Rapidity()),gen3mu->Pt(), effcorr*weight); //and here
	    /*Some verifications
	    hp_corr->Fill(fabs(gen3mu->Rapidity()),gen3mu->Pt(), effcorr*weight);
	    hp_ratio->Fill(fabs(gen3mu->Rapidity()),gen3mu->Pt(), weight*(simple/effcorr));
	    hp_weight->Fill(fabs(gen3mu->Rapidity()),gen3mu->Pt(), weight);
	    */
  ...
  /*Hist of the corrections and ratio of the simple corrections/the more complicated correction
  //**************************************************************                                                                                                                                     //Draw Corrections
  TCanvas *cbis = new TCanvas("cbis","cbis",3000,1500);
  cbis->Divide(2,1);

  //TH2Poly* hp_correction = (TH2Poly*)hp_sel->Clone("hp_correction");
  cbis->cd(1);
  //  gPad->SetLogz();                                                                                                                                                                                
  //hp_sel->GetZaxis()->SetRangeUser(1,200);                                                                                                                                                           
  hp_corr->Divide(hp_weight);
  hp_corr->GetXaxis()->SetTitle("|Y^{vis}(B_{c})|");
  hp_corr->GetYaxis()->SetTitle("p_{T}^{vis}(B_{c})");
  hp_corr->SetTitle("Corrections");
  // hp_corr->GetZaxis()->SetRangeUser(0.6,2.);
  hp_corr->Draw("COLZ0");
  line1->Draw("same"); line2->Draw("same"); line3->Draw("same"); line4->Draw("same"); line5->Draw("same");
  ...
  //  hp_efficiency->GetZaxis()->SetRangeUser(0,1);//1e-4,1);                                                                                                                                         
  hp_ratio->Divide(hp_weight);
  hp_ratio->GetXaxis()->SetTitle("|Y^{vis}(B_{c})|");
  hp_ratio->GetYaxis()->SetTitle("p_{T}^{vis}(B_{c})");
  hp_ratio->SetTitle("Ratio des corrections");
  //hp_ratio->GetZaxis()->SetRangeUser(0.6,1.3);
  hp_ratio->Draw("COLZ0");
  line1->Draw("same"); line2->Draw("same"); line3->Draw("same"); line4->Draw("same"); line5->Draw("same");

  cbis->SaveAs("figs/corections.pdf");
  cbis->SaveAs("figs/corrections.png");
  */
  //**************************************************************
  //Grab acceptance map
  ...
  gPad->SetLogz();
  gPad->SetRightMargin(0.15);
  hp_acceff->GetZaxis()->SetRangeUser(0,0.45);
  ...
  TCanvas *ccorr = new TCanvas("ccorr","ccorr",1000,1000);
  hist_corr->GetXaxis()->SetTitle("Corrections 'compliquees'");
  hist_corr->Draw();
  ccorr->SaveAs("hist_corr.png");
  TCanvas *cratio = new TCanvas("cratio","cratio",1000,1000);
  hist_ratio->GetXaxis()->SetTitle("Ratio des corrections");
  hist_ratio->Draw();
  cratio->SaveAs("hist_ratio.png");
  TCanvas *cSF = new TCanvas("cSF","cSF",1000,1000);
  hist_SF->GetXaxis()->SetTitle("SF");
  hist_SF->Draw();
  cSF->SaveAs("hist_SF.png");
  TCanvas *cLH = new TCanvas("cLH","cLH",1000,1000);
  hist_LH->GetXaxis()->SetTitle("L/H");
  hist_LH->Draw();
  cLH->SaveAs("hist_LH.png");
  */
  //**************************************************************
  //Store maps
