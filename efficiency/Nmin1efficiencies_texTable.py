...
vart.append( [
    ...
    ("dz",
     "(7) $max_i(d_z(\\PGm_i))$ & $<6$~mm & {:.{prec0}f} & {:.{prec1}f} & {:.{prec2}f} & {:.{prec3}f}\\\\\n" ), 
    ("CorrM",
     "(8) $m_{{\\rm corr}}$ & $<"+cuts[0][6]+"\\GeV$ & {:.{prec0}f} & {:.{prec1}f} & {:.{prec2}f} & {:.{prec3}f}\\\\\n" ),
    ("dRsum",
     "(9) $\\sum\\Delta R(\\PGm_i,\\PGm_j)$ & $<"+cuts[0][7]+"$ & {:.{prec0}f} & {:.{prec1}f} & {:.{prec2}f} & {:.{prec3}f}\\\\\n" ),
    ("3global",
...
vart.append( [
    ...
    ("dz",
     "(7) $max_i(d_z(\\PGm_i))$ & $<6$~mm & {:.{prec0}f} & {:.{prec1}f} & {:.{prec2}f} & {:.{prec3}f}\\\\\n" ), 
    ("CorrM",
     "(8) $m_{{\\rm corr}}$ & $<"+cuts[1][6]+"\\GeV$ & {:.{prec0}f} & {:.{prec1}f} & {:.{prec2}f} & {:.{prec3}f}\\\\\n" ),
    ("dRsum",
     "(9) $\\sum\\Delta R(\\PGm_i,\\PGm_j)$ & $<"+cuts[1][7]+"$ & {:.{prec0}f} & {:.{prec1}f} & {:.{prec2}f} & {:.{prec3}f}\\\\\n" ),
    ("3global",
     "\\multicolumn{{2}}{{c||}}{{(10) 3 \\textit{{hybrid-soft}} in loose acc.}} & {:.{prec0}f} & {:.{prec1}f} & {:.{prec2}f} & {:.{prec3}f}\\\\\n" ),
    ("TrimuVProbDimuVProbTrimuVProbDimuVProbalpha3Dalpha2Dalpha2Dalpha3DtauSignif3DtauSignif2DtauSignif3DtauSignif2DdcaCorrMdRsumdz",
     "\\multicolumn{{2}}{{c||}}{{cuts (1) to (9)}} & \\textbf{{{:.{prec0}f}}} & \\textbf{{$>${:.{prec1}f}}} & \\textbf{{{:.{prec2}f}}} & \\textbf{{{:.{prec3}f}}}\\\\\n" )
