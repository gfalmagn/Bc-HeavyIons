    ...
    t_prefit_(nullptr)
{
   ...
   ;
    ...
    currentToy_=0; nToys=0; fitStatus_=0; mu_=0; muLoErr_=0; muHiErr_=0; mu2_=0; mu2LoErr_=0; mu2HiErr_=0; numbadnll_=-1; nll_nll0_=-1; nll_bonly_=-1; nll_sb_=-1;
...
bool FitDiagnostics::runSpecific(RooWorkspace *w, RooStats::ModelConfig *mc_s, RooStats::ModelConfig *mc_b, RooAbsData &data, double &limit, double &limitErr, const double *hint) {
  ...
  mu_=r->getVal();
  if(res_b && (res_b->floatParsFinal().find("r2"))!=NULL)
    mu2_=(dynamic_cast<RooRealVar*>(res_b->floatParsFinal().find("r2")))->getVal();
  if (t_fit_b_) {
      t_fit_b_->Fill();
      resetFitResultTrees(withSystematics);}
  ...
  if (res_s) {
      RooRealVar *rf2 = ((res_s->floatParsFinal().find("r2"))!=NULL)?( dynamic_cast<RooRealVar*>(res_s->floatParsFinal().find("r2")) ):NULL;
      if(rf2!=NULL && rf2) mu2_=rf2->getVal();
	...
	if(rf2){
	  mu2LoErr_=+(rf2->hasRange("err68") ? rf2->getMax("err68") - bestFitVal : rf2->getAsymErrorHi());
	  mu2HiErr_=-(rf2->hasRange("err68") ? rf2->getMin("err68") - bestFitVal : rf2->getAsymErrorLo());
	  mu2Err_  =rf2->getError();
	}
...
void FitDiagnostics::createFitResultTrees(const RooStats::ModelConfig &mc, bool withSys){
	 ...
	 std::string poi2Name = "r2";
	 int poi2 = (int)mc.GetParametersOfInterest()->getRealValue(poi2Name.c_str(),-1.9);
	 if(poi2!=-2){
	   t_fit_b_->Branch(poi2Name.c_str(),&mu2_,Form("%s/D",poi2Name.c_str()));
	   t_fit_sb_->Branch(poi2Name.c_str(),&mu2_,Form("%s/D",poi2Name.c_str()));
	 
	   t_fit_b_->Branch(Form("%sErr",poi2Name.c_str()),&mu2Err_,Form("%sErr/D",poi2Name.c_str()));
	   t_fit_sb_->Branch(Form("%sErr",poi2Name.c_str()),&mu2Err_,Form("%sErr/D",poi2Name.c_str()));

	   t_fit_b_->Branch(Form("%sLoErr",poi2Name.c_str()),&mu2LoErr_,Form("%sLoErr/D",poi2Name.c_str()));
	   t_fit_sb_->Branch(Form("%sLoErr",poi2Name.c_str()),&mu2LoErr_,Form("%sLoErr/D",poi2Name.c_str()));

	   t_fit_b_->Branch(Form("%sHiErr",poi2Name.c_str()),&mu2HiErr_,Form("%sHiErr/D",poi2Name.c_str()));
	   t_fit_sb_->Branch(Form("%sHiErr",poi2Name.c_str()),&mu2HiErr_,Form("%sHiErr/D",poi2Name.c_str()));	   
	 }
