...
makeWorkspace() {
    text2workspace.py datacard${BASENAME}.txt -o datacard${BASENAME}${SECONDSTEP}${HISTSYST}${BLINDY}.root -P HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel --PO 'map=.*Kin1/BcSig:r1[1,0,6]' --PO 'map=.*Kin2/BcSig:r2[1,0,6]' --channel-masks --keyword-value METAFITSYST=${SECONDSTEP}${HISTSYST} --keyword-value BLIND=${BLINDY} -v 0 > /dev/null
}
runCombineBase() {
...
if [ "$nominalOnly" = false ] ; then
    ...
    FREEZE=${FREEZE_OR}",rgx{prop_binBDT3Kin.*_bin.*}"
    printSyst
    eval $(runCombineBase)
    FITSYST="_autoMCstatsNoBDT23"
    FREEZE=${FREEZE_OR}",rgx{prop_binBDT3Kin.*_bin.*},rgx{prop_binBDT2Kin.*_bin.*}"
    printSyst
    eval $(runCombineBase)
    ...
    FITSYST="_autoMCstatsNoBDT3"
    FREEZE=${FREEZE_OR}",rgx{prop_binBDT3Kin.*_bin.*}"
    if [ "$SECONDSTEP" = true -a $COL = "PbPb" ] ; then
...
combine -M GoodnessOfFit --algorithm saturated datacard_PbPb_2bins_2ndStep_blindYields.root
combine -M GoodnessOfFit --algorithm saturated datacard_PbPb_2bins_2ndStep_blindYields.root --toysFrequentist -t 500 --seed 2235 -n _PbPb
...
combine -M MultiDimFit datacard_PbPb_2bins_2ndStep.root --algo grid --points 900 --setParameterRanges r1=0,3.:r2=0,2.5 -n _PbPb
combine -M MultiDimFit datacard_pp_2bins_2ndStep.root --algo grid --points 900 --setParameterRanges r1=0.6,1.7:r2=0.75,1.35 -n _pp
